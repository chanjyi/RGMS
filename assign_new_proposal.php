...
if (isset($_POST['assign_proposal'])) {
    ...
    if ($prop_id <= 0 || $reviewer_id <= 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Notify reviewer (Check preferences first)
            $rev_q = $conn->prepare("SELECT email, notify_system FROM users WHERE id = ?");
            $rev_q->bind_param("i", $reviewer_id);
            $rev_q->execute();
            $rev_res = $rev_q->get_result();

            if ($rev_res && $rev_res->num_rows > 0) {
                $reviewer_data = $rev_res->fetch_assoc();
                
                // Only insert if notify_system is 1 (ON)
                if ($reviewer_data['notify_system'] == 1) {
                    $rev_email = $reviewer_data['email'];
            ...
            log_activity(
                ...
                (int)$prop_id,
                "Assign Proposal",
                "Assigned proposal #$prop_id to reviewer_id=$reviewer_id"
            );
...
/* =========================
   2) HANDLE ASSIGN APPEAL CASES (FIXED)
   ========================= */
if (isset($_POST['assign_appeal'])) {
    ...
    if ($prop_id <= 0 || $reviewer_id <= 0) {
        ...
        // --- NEW VALIDATION: CHECK IF REVIEWER ALREADY REVIEWED THIS ---
        ...
        if ($check_hist->get_result()->num_rows > 0) {
            $message  = "Error: This reviewer has already evaluated this proposal. Please select a different reviewer for the appeal.";
            $msg_type = "error";
        } else {
            // Proceed if they are a fresh reviewer
            $stmt = $conn->prepare("
                INSERT INTO reviews (proposal_id, reviewer_id, status, assigned_date, type)
            ...
            if ($stmt->execute()) {
                ...
                if ($rev_res && $rev_res->num_rows > 0) {
                    ...
                    // Only send notification if they have it turned ON (1)
                    if ($reviewer_data['notify_system'] == 1) {
                        $rev_email = $reviewer_data['email'];
                ...
                log_activity(
                    ...
                    (int)$prop_id,
                    "Assign Appeal Case",
                    "Assigned appeal case #$prop_id to reviewer_id=$reviewer_id"
                );
...
// Appeals waiting assignment (Fetch 'PENDING_REASSIGNMENT' which means HOD approved the appeal)
// Ensure this matches the status set by your HOD approval logic
...
<section class="home-section">
    <a href="admin_page.php" class="btn-back" style="display: inline-flex; align-items: center; text-decoration: none; color: #3C5B6F; font-weight: 600; margin-bottom: 15px;">
